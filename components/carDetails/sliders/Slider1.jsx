...
const DEFAULT_IMAGES = [
  {
    src: "/assets/images/section/slider-listing1.jpg",
    alt: "Car image 1",
    width: 1245,
    height: 701,
  },
...
const FALLBACK_IMAGE_WIDTH = 1245;
const FALLBACK_IMAGE_HEIGHT = 701;

export default function Slider1({ images = [] }) {
  const [loadedDimensions, setLoadedDimensions] = useState({});

  const slides = useMemo(() => {
    const sourceImages = images.length > 0 ? images : DEFAULT_IMAGES;
  ...
  useEffect(() => {
    ...
    slides.forEach((slide) => {
      ...
      image.onload = () => {
        if (!isMounted || !image.naturalWidth || !image.naturalHeight) {
          return;
        }

        setLoadedDimensions((previous) => {
          const previousDimensions = previous[slide.src];

          if (
            previousDimensions?.width === image.naturalWidth &&
            previousDimensions?.height === image.naturalHeight
          ) {
            return previous;
          }
  ...
  return (
    ...
    >
      {slides.map((slide, i) => {
        const dimensions = loadedDimensions[slide.src] || slide;
